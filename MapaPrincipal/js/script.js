...
document.addEventListener('DOMContentLoaded', () => {
  const grid = [
    ...
    [
      100, 3, 10, 100, 10, 100, 100, 100, 10, 100, 10, 10, 10, 10, 10, 100, 10,
    ...
    [
      100, 10, 10, 10, 10, 2, 10, 10, 10, 100, 10, 100, 100, 100, 10, 100, 10, 10,
  ...
  let startNode = { x: 24, y: 27 };
  let endNode = { x: 6, y: 5};
  let custoTotal = 0;
  let celulaAtualIndex = 0;
  let percorrerMapaClicado = false;
  let entradaMaisProxima = null; // armazenar a entrada mais próxima
  let entradasDungeonsAtualizadas = 0; // sinalizar mudanças nas entradas de dungeons
  let fimDeJogo = false;
  let caminho = [];
  let celulasPercorridas = grid.map(row => row.map(() => false));
  
  let entradasDungeons = [
    { x: 39, y: 17, visitado: false}, // entrada Dungeon 1
    { x: 24, y: 1, visitado: false }, // entrada Dungeon 2
    { x: 5, y: 32, visitado: false } // entrada Dungeon 3
  ];
  ...
  // Função para encontrar a entrada mais próxima
  function encontrarEntradaDungeon() {
  ...
  function atualizarMapa() {
    const container = document.getElementById('mapaPrincipalContainer');
  ...
  function percorrerMapa() {
    percorrerMapaClicado = true;
    entradaMaisProxima = encontrarEntradaDungeon();
  
    const todasVisitadas = entradasDungeons.every(entrada => entrada.visitado);
  
    if (todasVisitadas) {
      caminho = astar.search(graph, graph.grid[startNode.x][startNode.y], graph.grid[endNode.x][endNode.y]);
      fimDeJogo = true;
    } else {
      caminho = astar.search(graph, graph.grid[startNode.x][startNode.y], graph.grid[entradaMaisProxima.x][entradaMaisProxima.y]);
    }
    iniciarPercurso();
  }
  
  // Função para entrar na dungeon
  function entrarDungeon() {
  ...
  document.getElementById('entrarDungeon').addEventListener('click', entrarDungeon);
